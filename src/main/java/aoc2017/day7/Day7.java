...
public class Day7 {
    ...
    private static final Logger log = LoggerFactory.getLogger("2017 - Day 7");

    public static void main(String[] args) {

        String[] input = FileUtils.getLinesAsArray("src/main/resources/2017/day7-nodes");

        ProgramTreeMapper mapper = new ProgramTreeMapper();
        Map<String, Node<Integer>> tree = mapper.buildTree(input);

        String root = mapper.findRoot(tree);

        log.info("Part 1 - Root program [{}]", root);

        log.info("Part 2 - Unbalanced Trees [{}]", mapper.findUnbalancedSubTrees(tree));
    }
}
