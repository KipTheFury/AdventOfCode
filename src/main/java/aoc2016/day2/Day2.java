 ...
 * to use the bathroom! Fancy office buildings like this one usually have keypad locks on their bathrooms, so you
 ...
 * dismay, the keypad is not at all like you imagined it. Instead, you are confronted with the result of hundreds of
...
public class Day2 {
    ...
    private static final Logger log = LoggerFactory.getLogger("2016 - Day2");

    public static void main(String[] args) {

        String[] directions = FileUtils.getLinesAsArray("src/main/resources/2016/day2-directions");
        KeypadNavigator nav = new KeypadNavigator();

        String code_square = nav.findCode_square(directions);

        log.info("Part 1 - Bathroom Code [{}]", code_square);

        String code_diamond = nav.findCode_diamond(directions);

        log.info("Part 2 - Bathroom Code [{}]", code_diamond);
    }
}
