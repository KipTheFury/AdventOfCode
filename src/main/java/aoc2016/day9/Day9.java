 ...
 * scanning it for interesting files, you find one file in particular that catches your attention. It's compressed
 ...
 * and repeat them 2 times. Then, continue reading the file after the repeated data. The marker itself is not
...
public class Day9 {
    ...
    private static final Logger log = LoggerFactory.getLogger("2016 - Day 9");

    public static void main(String[] args) {

        String[] uncompressedFile = FileUtils.getLinesAsArray("src/main/resources/2016/day9-file");
        String file = uncompressedFile[0];

        FileDecompressor decompressor = new FileDecompressor();

        long decompressedFileLength = decompressor.decompress(file, false);

        log.info("Part 1 - Decompressed File Length [{}]", decompressedFileLength);

        long part2 = decompressor.decompress(file, true);

        log.info("Part 2 - Decompressed File Length [{}]", part2);
    }
}
